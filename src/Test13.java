...
public class Test13 {
     ...
     * 【注意3：注意1是错的，见{@link #deleteNode2(ListNode, ListNode)}】
     *
     * @param head        链表表的头
...
//            return head;
        ...
        if (toBeDeleted.next == null) {// 在多个节点的情况下，如果删除的是最后一个元素
            // 找待删除元素的前驱
            ListNode tmp = head;
    ...
    /**
     * 给定单向链表的头指针和一个结点指针，定义一个函数在0(1)时间删除该结点。
     * 【注意：输入的待删除结点必须是待链表中的结点，否则会引起错误，这个条件由用户进行保证】
     *
     * @param head        链表表的头
     * @param toBeDeleted 待删除的结点
     * @return 删除后的头结点
     */
    public static void deleteNode2(ListNode head, ListNode toBeDeleted) {
...
//        printList(head);
        ...
        //deleteNode2测试
        deleteNode2(head, null); // 删除的结点为空
        printList(head);

        deleteNode2(head, head); // 删除头结点
        printList(head);

        deleteNode2(head, last); // 删除尾结点
        printList(head);

        deleteNode2(head, middle); // 删除中间结点
        printList(head);

        deleteNode2(head, node); // 删除的结点不在链表中
        printList(head);
    }
}
