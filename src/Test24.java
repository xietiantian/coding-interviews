...
public class Test24 {
     ...
     * 此方法只是上面一个方法的辅助实现，对于数组为null和数组长度为0的情况，执行结果并非相同】
    ...
    public static boolean verifySequenceOfBST(int[] sequence, int start, int end) {
        ...
        int i;
        for (i = start; i < end && sequence[i] < sequence[end]; i++) ;

        // 执行到此处[start, i-1]的元素都是小于根结点（sequence[end]）的
        // [start, i-1]可以看作是根结点的左子树

        // right用于记录第一个不小于根结点的元素的位置
        int right = i;

        // 接下来要保证[right, end-1]的所有元素都是大于根根点的【A】
        // 因为[right, end-1]是根结点的右子树
        for (; i < end && sequence[i] > sequence[end]; i++) ;

        // 如果【A】条件满足，那么一定有 i == end，
        // 如果不满足那说明根结点的右子树[right, end-1]中有小于等于根结点的元素，
        // 不符合二叉搜索树的定义，返回false
        if (i != end) {
            return false;
        }
